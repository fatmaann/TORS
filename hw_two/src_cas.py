def get_by_cas(node_data, key, lock):
    with lock:
        return node_data.get(key, None)


def set_by_cas(node_data, key, value, lock):
    with lock:
...
def pop_by_cas(node_data, key, lock):
    with lock:
        return node_data.pop(key, None)


def update_by_cas(node_data, key, expected_value, new_value, lock):
    with lock:
        current_value = node_data.get(key, None)
